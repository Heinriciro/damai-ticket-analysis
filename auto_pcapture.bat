...
::Start frida-server.
adb shell "cd /data/local/tmp & su -c setsid ./frida-server-16.1.4-android-x86_64"
...
@echo Kill frida-server - %FRIDA_PID%
...
::Pull the pcap out and remove
adb pull /data/local/tmp/tcp.pcap .\
adb shell "su -c rm -rf /data/local/tmp/tcp.pcap"
